...
const PlinPago = ({ route, navigation }) => {
  ...
  const copiarNumero = async () => {
    if (!numeros.plin) {
      Alert.alert("Error", "Número de Plin no disponible");
      return;
    }
    await Clipboard.setStringAsync(numeros.plin);
  ...
  return (
    <Box flex={1} bg="white" p={6}>
      ...
      <VStack space={4}>
        ...
        <Button bg={COLORS.verdeAgua} borderRadius={10} onPress={seleccionarImagen}>
